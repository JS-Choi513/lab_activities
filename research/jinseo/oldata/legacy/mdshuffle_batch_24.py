...
for i in range(THREAD_NUM):
      ...
      if i < 5:
            range_pairs_per_thread.append((start_idx, end_idx, randomidx[i],32))
      else: 
            range_pairs_per_thread.append((start_idx, end_idx, randomidx[i],32))
...
test_ds = tf.data.Dataset.from_tensor_slices((batching_testimg,batching_testlab)).batch(32,num_parallel_calls=6)
...
def multi_epoch(nth_epoch, threadnum, train_arr,label_arr):
      ...
      with ProcessPoolExecutor(max_workers=threadnum) as executor:
       #     offset_idx = 0
        ...
        #          offset_idx +=1
        #          end_idx = offset_idx*divide_range
                  futures.append(executor.submit(part_epoch, range_pairs_per_thread[exec_threadnum][0], range_pairs_per_thread[exec_threadnum][1], range_pairs_per_thread[exec_threadnum][2], range_pairs_per_thread[exec_threadnum][3]))                         
...
def part_epoch(start, end, random_idx, batch_size):
