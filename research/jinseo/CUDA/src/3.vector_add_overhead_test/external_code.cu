...
void vecAdd(const float *h_A, const float *h_B, float *h_C, int numElements, int blocknum, int threadnum);
...
//    }
    ...
    int blocknum = strtol(argv[2], NULL, 10);
    int threadnum = strtol(argv[3], NULL, 10);
    ...
    vecAdd(h_A, h_B, h_C, numElements, blocknum, threadnum);
...
void vecAdd(const float *h_A, const float *h_B, float *h_C, int numElements, int blocknum, int threadnum)
{
    ...
    vecAddKernel<<<blocknum, threadnum>>>(d_A, d_B, d_C, numElements);
    CUDA_CHECK(cudaDeviceSynchronize());
    ...
    printf("Performance= %.2f GFlop/s, Time= %.3f msec, Size = %.0f Ops, "
           ...
           gigaFlops, msecTotal, flopsPerVecAdd, threadnum);
