...
#df_train = pd.read_csv("/home/js/fashion_mnist/fashion-mnist_train.csv")
#df_test = pd.read_csv("/home/js/fashion_mnist/fashion-mnist_test.csv")
'''
...
def multi_shuffling(train_path, test_path, train_arr,label_arr,threadnum): # file path 
      train_dir = csv.reader(open(train_path,'rt'))
      test_dir = csv.reader(open(test_path,'rt'))
      train_data_size = len(list(train_dir))#파일길이            
      global divide_range
      divide_range = int(train_data_size/threadnum)
      global train
      train = train_arr
      global label
      label = label_arr
...
for epoch in range(EPOCHS):
  train_ds = tf.data.Dataset.from_tensor_slices(multi_shuffling(x_train,y_train,100)).batch(32)
