...
arr_result = np.zeros((50))#np.arange(50)#tf.convert_to_tensor(arr)
#shuffled_train_result = np.zeros((540000,784,1))
...
def multi_shuffling_fixed(train_arr ,threadnum=10):
      print("input data type: ",type(train_arr))
      ...
      return arr_result#tf.convert_to_tensor(arr_result)
...
def multi_shuffling(train_arr ,threadnum=10):
      ...
      with ThreadPoolExecutor(max_workers=threadnum) as executor:
            ...
            for exec_threadnum in range(threadnum):
                  ...
                  offset_idx +=1
                  end_idx = offset_idx*divide_range
                  executor.submit(part_shuffle2,start_idx, end_idx, randomidx[exec_threadnum])                
      return arr_result

def part_shuffle2(start, end, random_idx):
      suffleidx = np.arange(divide_range)
      np.random.shuffle(suffleidx)    
      start_point = divide_range*random_idx
      end_point = start_point+divide_range
...
print(arrt[1:3])
'''
input_data = multi_shuffling(arr,10)
def __call__(inputdata):
    return multi_shuffling(input_data)
#print(input_data)
dataset = tf.data.Dataset.from_tensor_slices(tf.numpy_function(func = multi_shuffling_fixed, inp = [arr_result],Tout=tf.int64)).batch(10)
#shuffled = dataset.map(lambda x: tf.numpy_function(func = multi_shuffling_fixed, inp = [x],Tout=tf.int64)).batch(10)
#shuffled = dataset.map(tf.numpy_function(func = multi_shuffling_fixed, inp = [input_data],Tout=tf.int64)).batch(10)

#shuffled = dataset.map(__call__).batch(5)
